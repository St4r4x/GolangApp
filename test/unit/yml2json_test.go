...
`
	...
	err := ioutil.WriteFile(tempFile, []byte(invalidYAML), 0644)
	if err != nil {
		t.Fatalf("Failed to create test YAML file: %v", err)
	}
...
`
	...
	err := ioutil.WriteFile(tempFile, []byte(complexYAML), 0644)
	if err != nil {
		t.Fatalf("Failed to create test YAML file: %v", err)
	}
	defer os.Remove(tempFile)
...
`,
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			err := ioutil.WriteFile(tempFile, []byte(tc.yamlContent), 0644)
			if err != nil {
